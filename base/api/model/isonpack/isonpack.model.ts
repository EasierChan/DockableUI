...
export class ISONPack2Header extends Message {
    static readonly len = 16;
    flag1: number = 0xAC;       // char
    flag2: number = 0x3F;       // char
    packversion: number = 0x14; // char
    bitmap: number = 0x40;
    // data7: number = 0;              // bit
    // data6: number = 0;              // bit
    // data5: number = 0;              // bit
    ...
    packlen: number = 16;            // int
    appid: number = 0;              // short
    packid: number = 0;             // short
    packsn: number = 0;             // int
    ...
    toBuffer(): Buffer {
        let offset = 0;
        let buf = Buffer.alloc(ISONPack2Header.len, 0);
        buf.writeUInt8(this.flag1, offset); offset += 1;
        buf.writeUInt8(this.flag2, offset); offset += 1;
        buf.writeUInt8(this.packversion, offset); offset += 1;
        buf.writeUInt8(this.bitmap, offset); offset += 1;

        buf.writeUInt32LE(this.packlen, offset); offset += 4;
        buf.writeUInt16LE(this.appid, offset); offset += 2;
        buf.writeUInt16LE(this.packid, offset); offset += 2;
        buf.writeUInt32LE(this.packsn, offset); offset += 4;
        return buf;
    }
    ...
    fromBuffer(buf: Buffer, offset: number = 0): number {
        ...
        this.bitmap = buf.readUInt8(offset); offset += 1;
        this.packlen = buf.readUInt32LE(offset); offset += 4;
...
export class ISONPack2 extends Message {
    ...
    fromBuffer(buf: Buffer): number {
        ...
        if (buf.length < ISONPack2Header.len) {
            logger.warn(`ISONPack2: buffer length less than header len, ${buf.length} < ${this.head.packlen}`);
            return 0;
        }

        this.head.fromBuffer(buf);
        offset += ISONPack2Header.len;

        if (buf.length < this.head.packlen) {
            logger.warn(`ISONPack2: buffer length less than packlen, ${buf.length} < ${this.head.packlen}`);
            return 0;
        }
        ...
        if (this.head.bitmap & 0x40) {
            ...
            try {
                this.content = JSON.parse(buf.slice(offset, buf.indexOf(0, offset)).toString());
    ...
    toBuffer(): Buffer {
        ...
        if (this.content) {
            ...
            buf.writeUInt32LE(crcCode >>> 0, 0);
            contentBuf = Buffer.concat([contentBuf, buf], this.head.packlen);
            return contentBuf;
        } else {
            this.head.packlen = ISONPack2Header.len;
            return this.head.toBuffer();
        }
    }
}


function crc32(crc: number, str: Buffer) {
    let crc_table = crc32table();
    crc ^= -1;
