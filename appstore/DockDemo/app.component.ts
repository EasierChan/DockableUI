...
export class AppComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        btn_load.OnClick = () => {
            if (this.strategyStatus !== EStrategyStatus.STRATEGY_STATUS_WATCH) {
                alert("策略必须处于Watch状态！");
                return;
            }
    ...
    showStrategyInfo(data: any) {
        ...
        for (let i = 0; i < data.length; ++i) {
            ...
            this.strategyStatus = data[i].status;
    ...
    updateBasketPosition(data: any) {
        ...
        this.portfolioTable.rows.forEach(item => {
            portfolioMap[item.cells[0].Data] = item;
        });
    ...
    updateBasketExt(data: any) {
        ...
        this.basketTable.rows.forEach(item => {
            basketMap[item.cells[0].Data] = item;
        });
    ...
    createBookView(bookviewID) {
        ...
        if (this.appStorage && this.appStorage.bookView && this.appStorage.bookView.hasOwnProperty(bookviewID)) {
            ...
            if (code) {
                ...
                bookviewer.selectedItem = { symbolCode: codeInfo[code].SecuCode, code: codeInfo[code].InnerCode };
        ...
        bookviewer.onCellDBClick = (cellItem, cellIdx, rowIndex, row) => {
            if (bookviewer.selectedItem) {
                this.txt_UKey.Text = bookviewer.selectedItem.code;
